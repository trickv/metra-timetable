...
def parse_train_number(trip_id):
    """Parse train number from trip_id like 'UP-W_UW10_V1_C' -> '10'"""
    match = re.search(r'UW(\d+)', trip_id)
        ...
        for dest_stop_id in stop_ids:
            if dest_stop_id != chicago_stop_id:
                ...
                try:
                    ...
                    train_number = parse_train_number(trip_id)
                    train_label = f" (Train {train_number})" if train_number else ""
                    
                    if direction == 1:  # To Chicago
                        time_display = f"{dest_time} -> {chicago_time}{train_label}"
                    else:  # From Chicago
                        time_display = f"{chicago_time} -> {dest_time}{train_label}"
